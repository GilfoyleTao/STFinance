...
export class Utils {
  ...
  static foreachArray(id, arr): boolean {
    let bool = arr.some(function (ele, i, array) {
      if (ele.AccepterID == id) {
        return true;
      }
      return false;
    });
  ...
  static isEmpty(value): boolean {
    return value == null || typeof value === 'string' && value.length === 0 || typeof value === 'object' && value.length === 0
  }
  ...
  static getUrlParam(url) {
    ...
    if (urlArr[1]) {
      var shortUrl = urlArr[1];
      code = shortUrl.split('&state=')[0];
    }
    return code
  }
  static isArray(arr){
  ...
  //判断指定时间是否在时间段内
  static IsBookingFood(start,end):boolean{
    let today = new Date();
    start = start.split(':')[0]*60*60 + start.split(":")[1]*60;
    end = end.split(':')[0]*60*60 + end.split(":")[1]*60;
